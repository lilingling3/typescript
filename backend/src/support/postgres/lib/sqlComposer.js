 ...
 * @returns {*}
 */
function _filterToWhereClause(filter, needPushArgs) {
    var args = [];
    var clausePartial = "";
    var keyIndex = 1;
    for (var fieldName in filter) {
        ...
        if (descType == "string") {
            ...
            operatorCodeArr.forEach(operatorCode => {
                if (keyIndexForOPCode != 1 || keyIndex != 1)
                    andStrForOPCode = " and "; //这里不能漏判断keyIndex
                ...
                if (needPushArgs) {
                    if (operatorCode == "$in") {
                        if (!Array.isArray(operand))
                            throw new Error('_filterToWhereClause:invalid operand(not an array) for operatorCode:$in in filter:' + JSON.stringify(filter));
                        if (operand.length > 0) {
                            ...
                            if (operandItemType == 'number' || operandItemType == "boolean") {
                            ...
                            else if (operandItemType == 'string') {
                                ...
                                throw new Error('_filterToWhereClause:invalid operandItemType:' + operandItemType + ' for operatorCode:$in in filter:' + JSON.stringify(filter));
                            }
                        }
                        else
                            clausePartial += '1=1';
                        ...
                        if (operandType == "string") {
                            ...
                            throw new Error("_filterToWhereClause:invalid operand type " + operandType + " in filter:" + JSON.stringify(filter));
                        }
                    }
                }
                else {
    ...
    var whereClause = "";
    if (clausePartial)
        whereClause = " where " + clausePartial;
    return whereClause;
}
/**
 *
...
function prepareSelectSql(tableName, filter, fields, needArgs) {
    ...
    sql += ' from "' + tableName + '"' + _filterToWhereClause(filter, needArgs);
    return { sql, args };
}
exports.prepareSelectSql = prepareSelectSql;
class JSONBSetClazz {
    constructor(items) {
        this.items = [];
        this.items = items;
 ...
 * items like: [ { path:[<string>,<number>,...], value:<string>|<object>, createMissing:<boolean> } ]
 ...
 */
function prepareUpdateSql(tableName, filter, toUpdate, returnFields, needArgs) {
    ...
    for (var fieldName in toUpdate) {
        ...
        if (val != null && val != undefined) {
            ...
            if (valType == 'string' || valType == 'object') {
                ...
                if (valType == 'object') {
                    if (val instanceof JSONBSetClazz)
                        val = jsonbSetToSql(fieldName, val), noPush = true;
                    else
                        val = JSON.stringify(val), jsonbFlag = '::jsonb';
                }
                if (args) {
                    if (noPush) {
...
exports.prepareUpdateSql = prepareUpdateSql;
/*
 update {table_name} set {field_name} = case id when 1 then '1' when 2 then '1' end where id in(1, 2);
 ...
 */
function prepareBatchUpdateSql(tableName, toUpdates, filterKey, returnFields) {
    if (!filterKey)
        filterKey = 'id';
    var sql = 'update "' + tableName + '" set';
    var args = [];
    ...
    toUpdates.forEach(up => {
        ...
        setKeys.forEach(key => {
            if (key != filterKey) {
                ...
                if (!subSetClause)
                    subSetClause = ' "' + key + '" = case ' + filterKey;
                var val = up[key];
                var valType = typeof val;
                if (valType == 'string' || valType == 'object') {
                    var jsonbFlag = '', noPush = false;
                    if (valType == 'object') {
                        if (val instanceof JSONBSetClazz)
                            val = jsonbSetToSql(key, val), noPush = true;
                        else
                            val = JSON.stringify(val), jsonbFlag = '::jsonb';
                    }
                    if (noPush) {
    ...
    var valsOfInClause = toUpdates.map(v => {
        var val = v[filterKey];
        if (!val)
            throw new Error('prepareBatchUpdateSql:one element has no key:' + filterKey + ' as a filter in where clause,please check your update list!');
        var valType = typeof (val);
        if (valType == 'string') {
            args.push(val);
            return "$" + (args.length);
...
exports.prepareBatchUpdateSql = prepareBatchUpdateSql;
function prepareInsertSql(tableName, toInsert, returnFields, needArgs) {
...
exports.prepareInsertSql = prepareInsertSql;
/**
 ...
 * @returns {*}
 */
function prepareBatchInsertSql(tableName, toInserts, returnFields, needArgs) {
    ...
    fieldList.forEach(function (f) {
        let endSymbol = ',';
        if (fieldNameCount == elemKeyIndex)
            endSymbol = ')';
    ...
    for (var i = 0; i < insertsCount; i++) {
        ...
        var keyCount = Object.keys(toInsert).length;
        if (keyCount != fieldNameCount)
            throw new Error('prepareBatchInsertSql:one element\'s key count:' + keyCount + ' not equal first element:' + fieldNameCount);
        var keyIndex = 1;
        fieldList.forEach(function (f) {
            let endSymbol = ',';
            if (keyIndex == 1)
...
exports.prepareBatchInsertSql = prepareBatchInsertSql;
...
exports.prepareDeleteSql = prepareDeleteSql;
