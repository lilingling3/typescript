  ...
  for (var fieldName in filter) {
    ...
    if (descType == "string") {
      ...
      operatorCodeArr.forEach(operatorCode=>{
        ...
        if (needPushArgs) {
          if (operatorCode == "$in") {
            if(!Array.isArray(operand))throw new Error('_filterToWhereClause:invalid operand(not an array) for operatorCode:$in in filter:'+ JSON.stringify(filter));
            if(operand.length >0){
 ...
 * @returns {*}
 */
function prepareSelectSql(tableName:string, filter:any, fields:string[], needArgs:boolean|undefined) {
  var sql = 'select ';
  var args = [];
...
interface Item{
  path:(string | number)[];
  value:string | object;//如果传入的是object，会序列化成字符串
  createMissing?:boolean;//对于postgres来说,createMissing默认是true
}
 ...
 */
function prepareUpdateSql(tableName:string, filter, toUpdate, returnFields:string[], needArgs:boolean|undefined) {
...
export {
  prepareSelectSql,
  prepareUpdateSql,
  prepareBatchUpdateSql,
  prepareInsertSql,
  prepareBatchInsertSql,
  prepareDeleteSql,
  jsonbSet
};
